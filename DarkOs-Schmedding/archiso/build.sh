...
make_packages() {
    ...
    echo "###################################################################"
    mkarchiso ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "$(grep -h -v '^#' "${script_path}/packages.x86_64"| sed ':a;N;$!ba;s/\n/ /g')" install
}
...
make_customize_airootfs() {
    ...
    cp ${script_path}/darkos-mirrorlist ${work_dir}/x86_64/airootfs/etc/pacman.d/
    ...
    /usr/bin/lynx -dump -nolist 'https://wiki.archlinux.org/index.php/Installation_Guide?action=render' >> ${work_dir}/x86_64/airootfs/root/install.txt
...
make_boot_extra() {
    ...
    #[ ! -f ${work_dir}/x86_64/airootfs/usr/share/licenses/intel-ucode/LICENSE ] && 
    touch ${work_dir}/x86_64/airootfs/usr/share/licenses/intel-ucode/LICENSE
    ...
    touch ${work_dir}/x86_64/airootfs/usr/share/licenses/amd-ucode/LICENSE
